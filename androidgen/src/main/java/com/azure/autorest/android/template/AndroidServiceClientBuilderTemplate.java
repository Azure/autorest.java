...
public class AndroidServiceClientBuilderTemplate extends ServiceClientBuilderTemplate {
    ...
    public void write(ServiceClient serviceClient, JavaFile javaFile) {
        ...
        final Constructor serviceClientCtr = serviceClient.getConstructors().get(0); // AndroidServiceClient has only
                                                                                     // single Ctr.
        ...
        javaFile.javadocComment(comment -> {
            comment.description(String.format("A builder for creating a new instance of the %1$s type.",
        ...
        javaFile.publicFinalClass(serviceClientBuilderName, classBlock -> {
            ...
            serviceClient.getProperties().stream().filter(p -> !p.isReadOnly()).forEach(p -> {
                writeBuilderProperty(settings, classBlock, serviceClientBuilderName, p.getDescription(), p.getName(),
                        p.getType());
            });

            // Add class level variables and setters for all common properties.
            // e.g. ServiceClient.Builder, Credential Interceptor
            commonProperties.stream().forEach(p -> {
                writeBuilderProperty(settings, classBlock, serviceClientBuilderName, p.getDescription(), p.getName(),
                        p.getWireType());
            });

            String buildMethodName = this.primaryBuildMethodName(settings);
            JavaVisibility visibility = settings.shouldGenerateSyncAsyncClients() ? JavaVisibility.Private
            ...
            classBlock.method(visibility, null, String.format("%1$s %2$s()", buildReturnType, buildMethodName),
                    function -> {
                        ...
                        commonProperties.stream().filter(p -> p.getWireType() != ClassType.AndroidOkHttpInterceptor)
                                .forEach(p -> {
                                    // 1. Collect ServiceClient Ctr args.
                                    if (isBuilderType(p)) {
                        ...
                        function.line(String.format("%1$s client = new %2$s(%3$s);", serviceClient.getClassName(),
                                serviceClient.getClassName(), constructorArgsStr));
            ...
            if (JavaSettings.getInstance().shouldGenerateSyncAsyncClients()) {
                for (AsyncSyncClient asyncClient : asyncClients) {
                    ...
                    classBlock.javadocComment(comment -> {
                        comment.description(
                                String.format("Builds an instance of %1$s async client", asyncClient.getClassName()));
                        comment.methodReturns(String.format("an instance of %1$s", asyncClient.getClassName()));
                    });
                    classBlock.publicMethod(String.format("%1$s build%2$s()", asyncClient.getClassName(),
                            singleBuilder ? "AsyncClient" : asyncClient.getClassName()), function -> {
                                if (wrapServiceClient) {
                                    function.line("return new %1$s(%2$s());", asyncClient.getClassName(),
                                            buildMethodName);
                                } else {
                                    function.line("return new %1$s(%2$s().get%3$s());", asyncClient.getClassName(),
                                            buildMethodName, CodeNamer.toPascalCase(
                                                    asyncClient.getMethodGroupClient().getVariableName()));
                                }
                            });
                }
                ...
                for (AsyncSyncClient syncClient : syncClients) {
                    ...
                    classBlock.javadocComment(comment -> {
                        comment.description(
                                String.format("Builds an instance of %1$s sync client", syncClient.getClassName()));
                        comment.methodReturns(String.format("an instance of %1$s", syncClient.getClassName()));
                    });
                    classBlock.publicMethod(String.format("%1$s build%2$s()", syncClient.getClassName(),
                            singleBuilder ? "Client" : syncClient.getClassName()), function -> {
                                if (wrapServiceClient) {
                                    function.line("return new %1$s(%2$s());", syncClient.getClassName(),
                                            buildMethodName);
                                } else {
                                    function.line("return new %1$s(%2$s().get%3$s());", syncClient.getClassName(),
                                            buildMethodName, CodeNamer
                                                    .toPascalCase(syncClient.getMethodGroupClient().getVariableName()));
                                }
                            });
                }
            }
    ...
    protected String primaryBuildMethodName(JavaSettings settings) {
        return settings.shouldGenerateSyncAsyncClients() ? "buildInnerClient" : "buildClient";
    }

    private List<ClientMethodParameter> commonPropertiesFromCtr(final Constructor constructor) {
        return constructor.getParameters().stream().map(p -> {
            if (p.getWireType() == ClassType.AndroidRestClient) {
                return new ClientMethodParameter.Builder().description("The Azure Core generic ServiceClient Builder.")
                        .isFinal(false).wireType(ClassType.AndroidRestClientBuilder).name("serviceClientBuilder")
                        .isRequired(true).isConstant(false).fromClient(true).annotations(new ArrayList<>())
                        .defaultValue("new ServiceClient.Builder()").build();
            } else {
                return p;
            }
    ...
    private static void writeBuilderProperty(JavaSettings settings, JavaClass classBlock, String builderName,
            String propDescription, String propName, IType propType) {
        ...
        classBlock.publicMethod(
                String.format("%1$s %2$s(%3$s %4$s)", builderName, CodeNamer.toCamelCase(propName), propType, propName),
                function -> {
    ...
    private void setupRestClientBuilder(JavaBlock function, ServiceClient serviceClient,
            List<ClientMethodParameter> commonProperties, ClientMethodParameter restClientBuilder) {
            ...
            if (!anyHostParamAbsentExpression.isEmpty()) {
                function.ifBlock(anyHostParamAbsentExpression, illegalArgBlock -> {
                    illegalArgBlock.line("throw new IllegalArgumentException(\"Missing required parameters '%s'.\");",
                            String.join(", ", hostMapping.getHostParams().stream().map(h -> h.getName())
                                    .collect(Collectors.toList())));
                });
            }
        ...
        Optional<ClientMethodParameter> credInterceptorOpt = commonProperties.stream()
                .filter(p -> p.getWireType() == ClassType.AndroidOkHttpInterceptor).findFirst();
        ...
        if (credInterceptorOpt.isPresent()) {
            ...
            function.ifBlock(String.format("%s != null", credInterceptor.getName()), ifBlock -> {
                ifBlock.line("%s.setCredentialsInterceptor(%s);", restClientBuilder.getName(),
                        credInterceptor.getName());
            });
        }
    }
}
