...
public class AndroidServiceClientTemplate extends ServiceClientTemplate {
    ...
    public void write(ServiceClient serviceClient, JavaFile javaFile) {
        ...
        final AndroidEmbeddedBuilderTemplate embeddedBuilderTemplate = settings.shouldGenerateSyncAsyncClients() ? null
        ...
        final JavaVisibility visibility = serviceClient.getPackage().equals(
                ClientModelUtil.getServiceClientBuilderPackageName(serviceClient)) ? JavaVisibility.PackagePrivate
                        : JavaVisibility.Public;

        javaFile.javadocComment(comment -> {
            comment.description(
                    String.format("Initializes a new instance of the %1$s type.", serviceClient.getInterfaceName()));
        });
        javaFile.publicFinalClass(serviceClientClassDeclaration, classBlock -> {
            if (settings.shouldClientLogger()) {
                classBlock.privateFinalMemberVariable(ClassType.ClientLogger.toString(),
                        String.format("logger = new ClientLogger(%1$s.class)", serviceClient.getClassName()));
            }
            ...
            classBlock.packagePrivateFinalVariable(
                    "SerializerAdapter serializerAdapter = SerializerAdapter.createDefault()");
            ...
            ctrParamsSet1.forEach(p -> {
                ...
                classBlock.publicMethod(
                        String.format("%1$s get%2$s()", p.getClientType(), CodeNamer.toPascalCase(p.getName())),
                        function -> {
            ...
            serviceClient.getProperties().forEach(property -> {
                ...
                classBlock.publicMethod(
                        String.format("%1$s get%2$s()", property.getType(), CodeNamer.toPascalCase(property.getName())),
                        function -> {
            ...
            final String ctrParamsAsString = Stream
                    .concat(ctrParamsSet1.stream().map(ClientMethodParameter::getDeclaration),
            ...
            classBlock.constructor(visibility,
                    String.format("%1$s(%2$s)", serviceClient.getClassName(), ctrParamsAsString), constructorBlock -> {
                        ...
                        serviceClient.getProperties().stream().filter(ServiceClientProperty::isReadOnly).forEach(p -> {
                            if (p.getDefaultValueExpression() != null) {
                                constructorBlock.line("this.%1$s = %2$s;", p.getName(), p.getDefaultValueExpression());
                            }
                        });
                        ...
                        if (serviceClient.getProxy() != null) {
                            constructorBlock.line("this.service = serviceClient.getRetrofit().create(%s.class);",
            ...
            for (ClientMethod clientMethod : serviceClient.getClientMethods()) {
                ...
                if (clientMethodType == ClientMethodType.PagingAsync
                    ...
                    if (settings.getSyncMethods() == JavaSettings.SyncMethodsGeneration.ALL) {
                        PageResponseRetrieverTemplate pageResponseRetrieverTemplate = new PageResponseRetrieverTemplate(
                                clientMethod, clientMethod.getMethodPageDetails().getNextMethod(),
            ...
            classBlock.method(JavaVisibility.PackagePrivate, modifiers,
                    "String readAsString(okhttp3.ResponseBody body)", methodBodyBlock -> {
                        methodBodyBlock.ifBlock("body == null",
                    ...
                    methodBodyBlock -> {
                        methodBodyBlock.ifBlock("type.equals(byte[].class)", actionBlock -> {
                            ...
                            actionBlock.tryCatch(tryBlock -> {
                                tryBlock.ifBlock("body.contentLength() == 0",
                                        bodyEmptyBlock -> bodyEmptyBlock.line("return null;"));
                        ...
                        }, exceptions, "ex", catchBlock -> catchBlock.line("throw new RuntimeException(ex);"), null);
                    });
            ...
            classBlock.method(JavaVisibility.PackagePrivate, modifiers,
                    "<T> retrofit2.Response<T> executeRetrofitCall(retrofit2.Call<T> call)", methodBodyBlock -> {
                        List<String> exceptions = new ArrayList<>();
                        exceptions.add("java.io.IOException");
                        methodBodyBlock.tryCatch(tryBlock -> tryBlock.methodReturn("call.execute()"), exceptions, "ex",
                                catchBlock -> catchBlock.line("throw new RuntimeException(ex);"), null);
                    });
            ...
            classBlock.privateStaticFinalVariable(
                    "java.util.Map<String, SerializerFormat> SUPPORTED_MIME_TYPES = new java.util.TreeMap<>(String.CASE_INSENSITIVE_ORDER)");
            classBlock.privateStaticFinalVariable(
                    "java.util.TreeMap<String, SerializerFormat> SUPPORTED_SUFFIXES = new java.util.TreeMap<>(String.CASE_INSENSITIVE_ORDER)");
            ...
            classBlock.method(JavaVisibility.PackagePrivate, modifiers,
                    "SerializerFormat resolveSerializerFormat(String mimeContentType)", methodBlock -> {
