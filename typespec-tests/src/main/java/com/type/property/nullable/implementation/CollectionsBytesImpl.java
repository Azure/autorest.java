...
public final class CollectionsBytesImpl {
    ...
    CollectionsBytesImpl(NullableClientImpl client) {
        this.service
            = RestProxy.create(CollectionsBytesService.class, client.getHttpPipeline(), client.getSerializerAdapter());
        this.client = client;
    }
     ...
     * @return model with collection bytes properties along with {@link Response} on successful completion of
     * {@link Mono}.
     */
     ...
     * @throws ResourceNotFoundException thrown if the request is rejected by server on status code 404.
     * @throws ResourceModifiedException thrown if the request is rejected by server on status code 409.
     * @return model with collection bytes properties along with {@link Response}.
     */
    @ServiceMethod(returns = ReturnType.SINGLE)
