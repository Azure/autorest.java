...
public class CustomBaseUriTests {
    ...
    public void getEmptyWithValidCustomUri() throws Exception {
        ...
        clientBuilder.buildClient().getEmpty("local", null);
    }

    @Test
    public void getEmptyWithInvalidCustomUriAccountName() throws Exception {
        try {
            clientBuilder.buildClient().getEmpty("bad", null);
            Assert.fail();
        }
    ...
    public void getEmptyWithEmptyCustomUriAccountName() throws Exception {
        try {
            clientBuilder.buildClient().getEmpty(null, null);
    ...
    public void getGoodBadGood() throws Exception {
        ...
        try {
            clientBuilder.host("host:3000").buildClient().getEmpty("local", null);
        } catch (Exception e) {
            Assert.fail(e.getMessage());
        }
        try {
            clientBuilder.host("badhost").buildClient().getEmpty("local", null);
            Assert.fail();
    ...
    public void getEmptyMultipleThreads() throws Exception {
        ...
        Thread t1 = new Thread(() -> {
            try {
                client1.buildClient().getEmpty("badlocal", null);
                fail();
            } catch (RuntimeException e) {
                latch.countDown();
            } catch (Exception e) {
        ...
        Thread t2 = new Thread(() -> {
            try {
                client1.buildClient().getEmpty("local", null);
                latch.countDown();
            } catch (Exception ex) {
                fail(ex.getMessage());
            }
