// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for
// license information.
//
// Code generated by Microsoft (R) AutoRest Code Generator.
// Changes may cause incorrect behavior and will be lost if the code is
// regenerated.

package fixtures.header.models;

import com.azure.core.implementation.annotation.Fluent;
import com.fasterxml.jackson.annotation.JsonProperty;
import java.time.LocalDate;

/**
 * Defines headers for responseDate operation.
 */
@Fluent
public final class HeaderResponseDateHeaders {
    /*
     * response with header values "2010-01-01" or "0001-01-01"
     */
    @JsonProperty(value = "value")
    private LocalDate value;

    /**
     * Get the value property: response with header values "2010-01-01" or
     * "0001-01-01".
     *
     * @return the value value.
     */
    public LocalDate value() {
        return this.value;
    }

    /**
     * Set the value property: response with header values "2010-01-01" or
     * "0001-01-01".
     *
     * @param value the value value to set.
     * @return the HeaderResponseDateHeaders object itself.
     */
    public HeaderResponseDateHeaders value(LocalDate value) {
        this.value = value;
        return this;
    }
}
