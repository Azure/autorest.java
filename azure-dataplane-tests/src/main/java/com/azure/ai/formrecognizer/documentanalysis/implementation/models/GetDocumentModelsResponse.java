...
public final class GetDocumentModelsResponse implements JsonSerializable<GetDocumentModelsResponse> {
    ...
    public static GetDocumentModelsResponse fromJson(JsonReader jsonReader) throws IOException {
        return jsonReader.readObject(reader -> {
            GetDocumentModelsResponse deserializedGetDocumentModelsResponse = new GetDocumentModelsResponse();
            while (reader.nextToken() != JsonToken.END_OBJECT) {
                String fieldName = reader.getFieldName();
                reader.nextToken();

                if ("value".equals(fieldName)) {
                    List<DocumentModelSummary> value
                        = reader.readArray(reader1 -> DocumentModelSummary.fromJson(reader1));
                    deserializedGetDocumentModelsResponse.value = value;
                } else if ("nextLink".equals(fieldName)) {
                    deserializedGetDocumentModelsResponse.nextLink = reader.getString();
                } else {
                    reader.skipChildren();
                }
            }
