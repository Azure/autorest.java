...
public final class Base64Url {
    ...
    private static String unquote(String string) {
        if (!ExtensionUtils.isNullOrEmpty(string)) {
    ...
    public static Base64Url encode(byte[] bytes) {
        if (bytes == null) {
            ...
            return new Base64Url(ExtensionUtils.encodeURLWithoutPadding(bytes));
        }
    }
    ...
    public byte[] encodedBytes() {
        return ExtensionUtils.clone(bytes);
    }
    ...
    public byte[] decodedBytes() {
        ...
        return ExtensionUtils.decodeURL(bytes);
    }
