...
public final class DictionarysImpl {
     ...
     */
    DictionarysImpl(AutoRestComplexTestServiceImpl client) {
        this.client = client;
            ...
            public void onResponse(Call<okhttp3.ResponseBody> call, retrofit2.Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    if (response.code() == 200) {
                        final DictionaryWrapper decodedResult;
                        try {
                            decodedResult = client.deserializeContent(response.headers(), response.body(), DictionaryWrapper.class);
                        } catch(Exception ex) {
        ...
        if (response.isSuccessful()) {
            if (response.code() == 200) {
                return new Response<>(response.raw().request(),
                                        response.code(),
                                        response.headers(),
                                        this.client.deserializeContent(response.headers(), response.body(), DictionaryWrapper.class));
            } else {
    ...
    public void putValid(Map<String, String> defaultProgram, final Callback<Void> callback) {
        ...
        complexBody.setDefaultProgram(defaultProgram);
